...
def parse_args():
    ...
    parser.add_argument('--value', '-i', type=str, default='', help='value to store / update, perfix the value with @ will open a file with value name and push its content as value')
    return parser.parse_args()

def extract_value(value):
    if value.startswith('@'):
        with open(value[1:], 'r') as fl:
            return fl.read()
    return value
